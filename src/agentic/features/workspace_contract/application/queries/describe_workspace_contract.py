...
class DescribeWorkspaceContract:
    ...
    def execute(self, project_root: Path):
        ...
        return self._policy.summarize_workspace_contract(
            ...
            self._workspace_reader.list_override_paths(project_root, layout=self._policy.layout),
            self._workspace_reader.list_project_specific_paths(project_root, layout=self._policy.layout),
            agentic_dir_exists=self._workspace_reader.agentic_dir_exists(project_root, layout=self._policy.layout),
            config_exists=self._workspace_reader.config_exists(project_root, layout=self._policy.layout),
        )