...
class SyncPolicy:
    ...
    def plan_shared_rule_changes(
        ...
        *,
        ...
        for shared_rule_path in self._sorted_shared_rule_paths(shared_rule_paths):
            destination_path = self.layout.shared_rule_destination(
            ...
            planned_changes.append(SyncChange(
                shared_rule_path=shared_rule_path, action=action))
    ...
    def summarize_workspace_contract(
        ...
        *,
        ...
        expected_paths = self.expected_shared_rule_paths(
            project_root, shared_rule_paths)
        existing_path_set = {path for path in existing_shared_rule_paths}
        present_paths = tuple(
            path for path in expected_paths if path in existing_path_set)
        missing_paths = tuple(
            path for path in expected_paths if path not in existing_path_set)
