...
class SharedRulePath:
    ...
    def __post_init__(self) -> None:
        ...
        if ".." in normalized_path.parts:
            raise ValueError("Shared rule paths must not escape the rules tree")
        if normalized_path.parts[0] == "rules":
            raise ValueError("Shared rule paths are relative to the rules tree")
        if normalized_path.parts[0] in {"overrides", "project-specific"}:
            raise ValueError("Shared rule paths must not point at repo-local extension directories")
