...
from ..domain import ArchitectureConfigError
...
class ConfigLoader:
    ...
    def resolve_path(
        ...
        if explicit_config_path:
            candidates.append(Path(explicit_config_path).expanduser().resolve())
    ...
    def read_text(self, path: Path) -> str:
        try:
            ...
            raise ArchitectureConfigError(f"Could not read config file: {path}") from exc

    def load_mapping(self, raw_text: str, config_path: Path) -> tuple[dict[str, object], str]:
        suffix = config_path.suffix.lower()
        if suffix not in {".yaml", ".yml"}:
            raise ArchitectureConfigError(f"Unsupported config format: {config_path.name}")

        try:
            raw_data = yaml.safe_load(raw_text) or {}
        except yaml.YAMLError as exc:
            raise ArchitectureConfigError(f"Invalid config syntax in {config_path}") from exc
