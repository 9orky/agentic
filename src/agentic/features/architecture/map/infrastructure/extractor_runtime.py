...
class SubprocessExtractorRuntime(ExtractorRuntime):
    def run(
        ...
        with as_file(resource) as extractor_path:
            command = [executable, str(extractor_path), str(project_root), json.dumps(exclusions)]
            try:
                ...
                raise CheckerError(
                    exc.stderr.strip() or f"Extractor failed: {spec.resource_name}"
                ) from exc
        ...
        try:
            ...
            raise CheckerError(
                f"Extractor output was not valid JSON: {spec.resource_name}"
            ) from exc
    ...
    def _resolve_extractor_command(self, command_name: str) -> str:
        if os.path.sep in command_name or command_name.startswith("."):
            ...
            raise CheckerError(f"Required runtime '{command_name}' is not available")
