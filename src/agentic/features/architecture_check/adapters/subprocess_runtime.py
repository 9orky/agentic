...
from ..contracts import CheckerError
from ..domain.extractor_contract import ExtractorContractError, ExtractionResult, validate_architecture_map
from ..ports import ExtractorSpec


class SubprocessExtractorRuntime:
    def run(self, spec: ExtractorSpec, project_root: Path, exclusions: list[str]) -> ExtractionResult:
        executable = _resolve_extractor_command(spec.command)
        ...
        try:
            return validate_architecture_map(parsed, spec.resource_name)
        except ExtractorContractError as exc:
            raise CheckerError(str(exc)) from exc


def _resolve_extractor_command(command_name: str) -> str:
