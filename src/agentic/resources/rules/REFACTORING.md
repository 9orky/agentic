...
This file is part of the durable `agentic/` collaboration contract, so refactor guidance here should describe stable migration rules rather than disposable local tactics.
...
A placeholder is a file or function that explicitly explains what must still be done for the final wiring.

Use placeholders only when they make the target structure visible without pretending that unfinished behavior already exists.

Placeholder rules:

1. a placeholder must say what final wiring is still required
2. a placeholder must stay local to the owning slice or module unless it is the deliberate public seam being prepared
3. a placeholder must not masquerade as completed behavior with fake logic, silent success, or hidden branching
4. a placeholder should be easy to replace once the real wiring is implemented
5. if a placeholder would become a long-lived boundary, stop and ask instead
