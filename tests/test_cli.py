...
class CliTests(unittest.TestCase):
    ...
    def test_llm_flag_bootstraps_and_prints_prompt(self) -> None:
        with TemporaryDirectory() as temp_dir:
            previous_cwd = Path.cwd()
            output = StringIO()
            try:
            ...
            rendered = output.getvalue()
            self.assertIn(
                "You are working in a project that uses agentic", rendered)
            self.assertIn("agentic/guide/WORKFLOW.md", rendered)
